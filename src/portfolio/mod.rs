...
impl Portfolio {
    ...
    /// Check if portfolio has any meaningful data for components
    pub fn has_data(&self) -> bool {
        ...
        if self.backtest_results.is_some() || self.strategy_signals.is_some() {
            return true;
        }
        false
    }
    ...
    pub fn get_position_value(&self, symbol: &str) -> Option<f64> {
        self.holdings.get(symbol).and_then(|_shares| {
