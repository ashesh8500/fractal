...
from portfolio_lib.models.strategy import (
    BacktestConfig, BacktestResult, StrategyConfig, Trade, TradeAction
)
...
class BacktestingService:
    ...
    def run_backtest(
        ...
        """
        self.logger.info(f"Starting backtest for strategy '{strategy_config.name}' from {backtest_config.start_date} to {backtest_config.end_date}")
        ...
        simulation_result = self._run_simulation(
            strategy, strategy_config, backtest_config, 
            initial_holdings, price_history
        )
    ...
    def _run_simulation(
        ...
        simulation_dates = [
            date for date in simulation_dates 
            if backtest_config.start_date.date() <= date.date() <= backtest_config.end_date.date()
        ]
        ...
        rebalance_days = self._get_rebalance_frequency_days(strategy_config.rebalance_frequency)
        ...
        for i, current_date in enumerate(simulation_dates):
            ...
            for symbol, shares in current_holdings.items():
                if symbol in current_prices:
                    ...
                    current_weights[symbol] = position_value / max(portfolio_value, 1e-10)
                else:
            ...
            if should_rebalance and i < len(simulation_dates) - 1:  # Don't rebalance on last day
                try:
                    ...
                    strategy_result = strategy.execute(
                        current_weights,
                        {symbol: df[df.index <= current_date] for symbol, df in price_history.items()},
                        current_prices,
                        strategy_config
                    )
                    ...
                    self.logger.warning(f"Strategy execution failed on {current_date}: {e}")
                    continue
        ...
        for trade in trades:
            ...
            if trade.action == TradeAction.BUY:
                ...
                if new_cash >= total_needed:
                    new_holdings[trade.symbol] = new_holdings.get(trade.symbol, 0) + trade.quantity
